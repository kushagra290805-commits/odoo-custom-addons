...
class McpProvider:
    ...
    """
    def __init__(self, config: McpConfiguration, global_transport: McpTransport, pool: Optional[SessionTransportPool] = None):
        self.config = config
        self.global_transport = global_transport
        self.pool = pool

    def _get_active_transport(self, context: ExecutionContext) -> McpTransport:
        if self.config.session_binding == 'none' or not self.pool:
            return self.global_transport
            
        field = self.config.session_binding_field
        if field not in self.config.allowed_request_context_fields:
            raise RuntimeException("FORBIDDEN_CONTEXT_FIELD", "Session binding field is not allowlisted.", f"Field {field} is required for session binding but not allowlisted.")
            
        request_context = getattr(context, 'request_context', {})
        session_identity = request_context.get(field)
        if not session_identity:
            raise RuntimeException("MISSING_SESSION_IDENTITY", "Request is missing required session identity.", f"Request context is missing {field}.")
            
        return self.pool.get_transport(session_identity)
    ...
    def _tools_list(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        result = self.global_transport.list_tools()
    ...
    def _tools_call(self, parameters: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        ...
        request_context = getattr(context, 'request_context', {})
        transport = self._get_active_transport(context)
        result = transport.call_tool(name, arguments, request_context=request_context)
