# -*- coding: utf-8 -*-
"""Phase 45 U1 — semantic capability reconciliation tests (ADR-0072 / F-01).

DB-free unit tests: env is faked; execution goes through a mocked canonical
router (post-ADR-0069 contract). Legacy payload-merge/isolation behaviors are
refreshed to the current architecture.
"""
import json
import unittest
...
def make_adapter(capabilities='', config=None, discovered=('search_code', 'get_file_contents'),
    ...
    connector = _Rec(connector_id='test_mcp', capability_ids=[_Rec(technical_name=t) for t in discovered])
