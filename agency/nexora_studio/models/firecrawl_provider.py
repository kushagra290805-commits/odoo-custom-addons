...
class FirecrawlProvider(models.AbstractModel):
    _name = 'nexora.provider.firecrawl'
    _description = 'Canonical Firecrawl Website Extraction Provider'
    ...
    def execute(self, request: ProviderExecutionRequest) -> ProviderExecutionResult:
        ...
        Uses the existing Firecrawl MCP connection when available.
        ...
        try:
            ...
            result = future.result(timeout=120.0) # Increased timeout for web scraping
            
            return ProviderExecutionResult(success=True, data=result, error=None, execution_ms=(time.time()-start_time)*1000)

        except Exception as e:
            _logger.error(f"Firecrawl MCP execution error: {e}")
            return ProviderExecutionResult(success=False, data=None, error=f"Firecrawl MCP error: {str(e)}", execution_ms=(time.time()-start_time)*1000)
