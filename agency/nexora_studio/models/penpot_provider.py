...
class PenpotProvider(models.AbstractModel):
    _name = 'nexora.provider.penpot'
    _description = 'Canonical Penpot Design Provider'

    @api.model
    def execute(self, request: ProviderExecutionRequest) -> ProviderExecutionResult:
        start_time = time.time()
        """
        Canonical execution interface for Penpot.
        Uses the existing MCP connection.
        """
        try:
            import asyncio
            ...
            _logger.error(f"Penpot MCP execution error: {e}")
            return ProviderExecutionResult(success=False, data=None, error=f"Penpot MCP error: {str(e)}", execution_ms=(time.time()-start_time)*1000)
